...
test("整合測試：複製 Markdown 轉換為 HTML", () => {
  ...
  const diagramFile = path.join(outDir, "assets", "diagrams", "diagram-0.png");
  assert.equal(fs.existsSync(diagramFile), true, "HTML 建置應產生 Mermaid 圖檔");

  // 驗證圖表被渲染，而且輸出為 Marp slide deck
  const html = fs.readFileSync(htmlFile, "utf-8");
  assert.match(html, /<img[^>]+class="mermaid-diagram"/, "HTML 應參照圖表圖片");
